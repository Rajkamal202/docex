...
// Check if an ID is a local proposal (includes both local- and template- prefixes)
export const isLocalProposal = (id: string) => id.startsWith(LOCAL_PREFIX) || id.startsWith(TEMPLATE_PREFIX)
...
export const getLocalProposal = (id: string): Proposal | null => {
  // First check local proposals storage
  const proposals = getLocalProposals()
  if (proposals[id]) {
    return proposals[id]
  }
  
  // Then check templates storage (for template- prefixed IDs)
  if (id.startsWith(TEMPLATE_PREFIX)) {
    try {
      const templatesStored = localStorage.getItem("lightnote_templates")
      if (templatesStored) {
        const templates = JSON.parse(templatesStored)
        const template = templates[id]
        if (template) {
          // Convert template to proposal format
          const assetsStored = localStorage.getItem("lightnote_assets")
          const assets = assetsStored ? JSON.parse(assetsStored) : {}
          
          // Import dynamically to avoid circular dependency
          const { templateToProposal } = require("@/lib/document-model")
          return templateToProposal(template, assets)
        }
      }
    } catch (error) {
...
export const saveLocalProposal = (proposal: Proposal): void => {
  ...
  // For template-prefixed IDs, also update the templates storage
  if (proposal.id.startsWith(TEMPLATE_PREFIX)) {
    try {
      const templatesStored = localStorage.getItem("lightnote_templates")
      const templates = templatesStored ? JSON.parse(templatesStored) : {}
      if (templates[proposal.id]) {
        // Update existing template
        const { proposalToTemplate } = require("@/lib/document-model")
        templates[proposal.id] = proposalToTemplate(proposal, templates[proposal.id])
        localStorage.setItem("lightnote_templates", JSON.stringify(templates))
      }
    } catch (error) {
  ...
  // Always save to proposals storage as well
  const proposals = getLocalProposals()
